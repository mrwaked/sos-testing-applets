...
3. Worker calls `wise_chat_messages_endpoint` with the cookie and returns `{ nowTime, fetchedAt, messages: [{ id, time, text }] }`
   newest-first, cached for 30 s, with `Access-Control-Allow-Origin: *`.
...
| -------------- | ------------- | ------------------------------- |
| open           | `10 samců`    | poslední hlášení HH:MM          |
| open, approx   | `15+ samců`   | poslední hlášení HH:MM          |
| open, unknown  | `? samců`     | poslední hlášení HH:MM          |
| closed         | `0 samců`     | zavřeli v HH:MM                 |
| stale (> 4 h)  | `0 samců`     | bez hlášení od HH:MM            |
